#!/usr/bin/env python3
"""
Multi-Baseline Analyzer - Hybrid RF + DTW Pipeline (Fixed Version)
...
from ml_service.dtw_scorer import score_window, DTW_FEATURE_ORDER
from schemas import BaselineResponse, BoxingStatusResponse
...
class MultiBaselineAnalyzer:
    ...
    def _load_enhanced_baselines(self):
        ...
        if not self.enhanced_baselines_dir.exists():
            logger.warning("Enhanced baselines directory not found")
            return
...
def get_analyzer() -> MultiBaselineAnalyzer:
    ...
    if _analyzer is None:
        ...
        baselines_dir = base_dir / "dataset"
        enhanced_baselines_dir = base_dir / "dataset_enhanced"
...
def get_system_status() -> Dict[str, any]:
    ...
    return {
        ...
        'enhanced_baselines_loaded': len(analyzer.enhanced_baselines) > 0,
        ...
        'total_enhanced_baselines': sum(len(baselines) for baselines in analyzer.enhanced_baselines.values())
    }
